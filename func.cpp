...
bool test_equation (char *file_name) {
    ...
    for (size_t i = 0; i < num_tests; i++) {
        ...
        size_t size_t_num_solutions;

        fscanf(tests_file, "%lg%lg%lg%zd", &tests[i].a, &tests[i].b, &tests[i].c, &size_t_num_solutions);
        initialize_NUM_ROOTS(&num_solutions[i], size_t_num_solutions);
        ...
        switch (num_solutions[i]) {
            ...
            case NAN_ROOTS : printf ("Test %zd: entered incorrect amount of solutions", i+1); my_exit;
            default        : printf ("Test %zd: entered incorrect amount of solutions", i+1); my_exit;
        }
    }
    ...
    if (tests_passed == num_tests) { printf (  "All %zd tests passed!\n\n",                num_tests); }
    else                           { printf ("%zd of %zd tests passed.\n\n", tests_passed, num_tests); }
...
bool one_test (size_t num_test, S_COEFFS coefficients, NUM_ROOTS num_solutions, double* solutions) {
    ...
    switch (num_x) {
        ...
        case ONE_ROOT: 
            ...
            if (double_equal (x1, solutions[0]) && isnan (x2)) {
                
                success_message (num_test);
                return true;
            } 

            else {
                
                error_message (num_test, coefficients, num_solutions, solutions, num_x, x1, x2);
                return false;
            }

        case TWO_ROOTS:
            
            if ((double_equal (x1, solutions[0]) && double_equal (x2, solutions[1])) || \
                (double_equal (x1, solutions[1]) && double_equal (x2, solutions[0])))  {
                ...
                error_message   (num_test, coefficients, num_solutions, solutions, num_x, x1, x2); return false;
            }
        ...
        case NAN_ROOTS: 
            ...
            printf ("Test %zd: entered incorrect amount of solutions", num_test);                                              return false;
...
void error_message (size_t num_test, S_COEFFS coefficients, NUM_ROOTS num_solutions, double* solutions, NUM_ROOTS num_x, double x1, double x2) {
    ...
    printf ("ERR in test %zd while solving equation ", num_test+1);
    if (double_equal (a, 1)) printf ("x^2") ; else if (double_equal (a, -1)) printf("-x^2");   else printf("%lgx^2",   a);
    if (double_equal (b, 1)) printf ("+x")  ; else if (double_equal (b, -1)) printf("-x")  ;   else printf("%+lgx",    b);
    if (double_equal (c, 0)) printf ("=0\n");                                                  else printf("%+lg=0\n", c);
...
void initialize_NUM_ROOTS (NUM_ROOTS *target, size_t value) {
    ...
    if      (value == 0) { *target = ZERO_ROOTS; }
    else if (value == 1) { *target = ONE_ROOT  ; }
    else if (value == 2) { *target = TWO_ROOTS ; }
    else if (value == 3) { *target = INF_ROOTS ; }
    else                 { *target = NAN_ROOTS ; }
}